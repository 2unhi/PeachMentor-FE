...
const RecordScript = ({ selectedDate }) => {
  ...
  useEffect(() => {
    ...
    const getCanSaveAnalysis = async () => {
      try {
        const response = await instance.get(
          `${SPRING_API_URL}/analysis/available`
        );
        if (response.data.isSuccess) {
          if (
            response.data.code === "STATISTICS2004" &&
            response.data.result.canSaveAnalysis
          ) {
            saveAnalysis();
          } else {
            console.error(
              "유저의 7일 레포트 생성 가능 여부 받아오기 서버 오류"
            );
          }
        } else {
          console.error("서버 에러");
  ...
  return (
    <div className="relative flex flex-col w-full min-h-screen overflow-hidden">
      ...
      {/* 컨텐츠 */}
      <div
        className="flex-1 w-full max-w-[500px] mx-auto overflow-y-auto px-4 space-y-6 bg-white"
        style={{
          paddingTop: "94px",
          paddingBottom: "70px",
        }}
      >
        ...
        {/* 강조 문구 */}
        {showGuide && (
          <div className="absolute flex items-center space-x-1 transform -translate-x-1/2 left-[calc(50%+17px)] top-12 animate-bounce">
